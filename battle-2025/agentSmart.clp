...
;  --- AGENTE DUE ------------------------------
;  ---------------------------------------------
(defrule print-agent-2 (declare (salience 40))
    ?f <- (flag to-be-printed)
=>
    (retract ?f)
    (printout t "Agente due" crlf)
)
...
;  --- nella cella vuota. --------------------------------------------
;  -------------------------------------------------------------------
(defrule fire-up-three-pieces (declare (salience -15))
	(status (step ?s) (currently running))
	(moves (fires ?nf&:(> ?nf 0)))
	(sunk-boats (four 0))
	(sure-guess (x ?x) (y ?y) (content ?p1&~water))
...
(defrule fire-down-three-pieces (declare (salience -15))
	(status (step ?s) (currently running))
	(moves (fires ?nf&:(> ?nf 0)))
	(sunk-boats (four 0))
	(sure-guess (x ?x) (y ?y) (content ?p1&~water))
...
(defrule fire-left-three-pieces (declare (salience -15))
	(status (step ?s) (currently running))
	(moves (fires ?nf&:(> ?nf 0)))
	(sunk-boats (four 0))
	(sure-guess (x ?x) (y ?y) (content ?p1&~water))
...
(defrule fire-right-three-pieces (declare (salience -15))
	(status (step ?s) (currently running))
	(moves (fires ?nf&:(> ?nf 0)))
	(sunk-boats (four 0))
	(sure-guess (x ?x) (y ?y) (content ?p1&~water))
...
;  -------------------------------------------------------------------
;  --- Se ho due pezzi di fila e poi una cella vuota e ho ------------
;  --- ancora barche da tre da trovare, allora provo una fire --------
;  --- nella cella vuota. --------------------------------------------
;  -------------------------------------------------------------------
(defrule fire-up-two-pieces (declare (salience -16))
    (status (step ?s) (currently running))
    (moves (fires ?nf&:(> ?nf 0)))
    (sunk-boats (three ?n&:(< ?n 2)))
    (sure-guess (x ?x) (y ?y) (content ?p1&~water))
    (sure-guess (x ?x1&:(eq ?x1 (- ?x 1))) (y ?y) (content ?p2&~water))
    (not (sure-guess (x ?x2&:(eq ?x2 (- ?x 2))) (y ?y)))
    ...
    (test (>= (- ?x 2) 0))
=>
    (printout t "Fire in cell [" (- ?x 2) ", " ?y "], because there are two pieces in a row and still need to find a three pieces boat." crlf)
    (assert (exec (step ?s) (action fire) (x (- ?x 2)) (y ?y)))
    (pop-focus)
)

(defrule fire-down-two-pieces (declare (salience -16))
    (status (step ?s) (currently running))
    (moves (fires ?nf&:(> ?nf 0)))
    (sunk-boats (three ?n&:(< ?n 2)))
    (sure-guess (x ?x) (y ?y) (content ?p1&~water))
...
(defrule fire-left-two-pieces (declare (salience -16))
    (status (step ?s) (currently running))
    (moves (fires ?nf&:(> ?nf 0)))
    (sunk-boats (three ?n&:(< ?n 2)))
    (sure-guess (x ?x) (y ?y) (content ?p1&~water))
    ...
    (test (>= (- ?y 2) 0))
=>
    (printout t "Fire in cell [" ?x ", " (- ?y 2) "], because there are two pieces in a row and still need to find a three pieces boat." crlf)
    (assert (exec (step ?s) (action fire) (x ?x) (y (- ?y 2))))
    (pop-focus)
)

(defrule fire-right-two-pieces (declare (salience -16))
    (status (step ?s) (currently running))
    (moves (fires ?nf&:(> ?nf 0)))
    (sunk-boats (three ?n&:(< ?n 2)))
    (sure-guess (x ?x) (y ?y) (content ?p1&~water))
...
;  -------------------------------------------------------------------
;  --- Se ho un pezzo e poi una cella vuota e ho ancora barche da ----
;  --- due da trovare, allora provo una fire nella cella vuota. ------
;  -------------------------------------------------------------------
(defrule fire-up-one-piece (declare (salience -17))
    (status (step ?s) (currently running))
    (moves (fires ?nf&:(> ?nf 0)))
    ...
    (test (>= (- ?x 1) 0))
=>
    (printout t "Fire in cell [" (- ?x 1) ", " ?y "], because there is a piece and still need to find a two pieces boat." crlf)
    (assert (exec (step ?s) (action fire) (x (- ?x 1)) (y ?y)))
    (pop-focus)
)

(defrule fire-down-one-piece (declare (salience -17))
    (status (step ?s) (currently running))
    (moves (fires ?nf&:(> ?nf 0)))
    (sunk-boats (two ?n&:(< ?n 3)))
    (sure-guess (x ?x) (y ?y) (content ?p1&~water))
...
(defrule fire-left-one-piece (declare (salience -17))
    (status (step ?s) (currently running))
    (moves (fires ?nf&:(> ?nf 0)))
    (sunk-boats (two ?n&:(< ?n 3)))
    (sure-guess (x ?x) (y ?y) (content ?p1&~water))
    ...
    (test (>= (- ?y 1) 0))
=>
    (printout t "Fire in cell [" ?x ", " (- ?y 1) "], because there is a piece and still need to find a two pieces boat." crlf)
    (assert (exec (step ?s) (action fire) (x ?x) (y (- ?y 1))))
    (pop-focus)
)

(defrule fire-right-one-piece (declare (salience -17))
    (status (step ?s) (currently running))
    (moves (fires ?nf&:(> ?nf 0)))
    (sunk-boats (two ?n&:(< ?n 3)))
    (sure-guess (x ?x) (y ?y) (content ?p1&~water))
...
;  -------------------------------------------------------------------
;  --- Infine fire nella cella più probabile data da: ----------------
;  --- num pezzi rimasti / num celle vuote, considerando -------------
;  --- sia righe che colonne pre trovare i sub rimasti. --------------
;  -------------------------------------------------------------------
(defrule fire-most-probable-cell (declare (salience -18))
	(status (step ?s) (currently running))
	(moves (fires ?nf&:(> ?nf 0)))
