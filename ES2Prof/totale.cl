% master.cl - fatti (prof, subject, propaedeutic)
% Questo file contiene solo i fatti; le regole e i vincoli sono in generaCalendarioSlot.cl

% Professors
prof("Muzzetto").
prof("Pozzato").
prof("Schifanella Rossano").
prof("Tomatis").
prof("Micalizio").
prof("Terranova").
prof("Mazzei").
...
prof("Taddeo").
prof("Gribaudo").
prof("Schifanella Claudio").
prof("Lombardo").
prof("Travostino").

% Subjects (Name, Professor, TotalHours)
...
subject("Linguaggi di markup","Schifanella Rossano",20).
...
subject("Progettazione e sviluppo di applicazioni web su dispositivi mobile I","Schifanella Rossano", 10).
subject("Progettazione e sviluppo di applicazioni web su dispositivi mobile II","Schifanella Claudio", 10).
...
% Propedeuticità (A is prerequisite of B)
...
propaedeutic("Linguaggi di markup", "Ambienti di sviluppo e linguaggi client-side per il web").
